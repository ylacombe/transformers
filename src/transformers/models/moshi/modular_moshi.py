...
class MoshiRMSNorm(GemmaRMSNorm):
...
class MoshiDecoderAttention(MistralAttention):
    pass


class MoshiDecoderFlashAttention2(MistralFlashAttention2):
    pass
...
MOSHI_DECODER_ATTENTION_CLASSES = {
    ...
    "sdpa": MoshiDecoderSdpaAttention,
}


class MoshiDecoderLayer(MistralDecoderLayer):
    def __init__(self, config: MoshiConfig, layer_idx: int):
        super().__init__()
...
class MoshiDepthDecoderAttention(nn.Module):
    ...
    def __init__(self, config: MoshiConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        query_states = self.q_proj(hidden_states, cache_position)
        key_states = self.k_proj(hidden_states, cache_position)
        value_states = self.v_proj(hidden_states, cache_position)
        ...
        attn_output = self.o_proj(attn_output, cache_position)
...
class MoshiDepthDecoderSdpaAttention(MoshiDepthDecoderAttention):
    """
...
MOSHI_DEPTH_DECODER_ATTENTION_CLASSES = {
    "eager": MoshiDepthDecoderAttention,
    "flash_attention_2": MoshiDepthDecoderFlashAttention2,
...
class MoshiDepthDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states, cache_position)
        hidden_states = residual + hidden_states
